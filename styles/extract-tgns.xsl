            ...
            <xsl:when test="string(.//a13c)">
                <xsl:element name="item">
                    ...
                    <xsl:element name="body">
                        <xsl:element name="entity"><xsl:value-of select="./kerndaten_id"/></xsl:element>
                        <xsl:element name="value"><xsl:value-of select=".//a13c"/></xsl:element>                    
                    </xsl:element>
                </xsl:element>
            </xsl:when>
            <xsl:when test="string(.//a13d)">
                <xsl:element name="item">
                    <xsl:element name="id">
                        <xsl:value-of select="$uid"/>
                    </xsl:element>
                    <xsl:element name="body">                
                        <xsl:element name="entity"><xsl:value-of select="./kerndaten_id"/></xsl:element>
                        <xsl:element name="value"><xsl:value-of select=".//a13d"/></xsl:element>                    
                    </xsl:element>
                </xsl:element>
            </xsl:when>
        </xsl:choose>
