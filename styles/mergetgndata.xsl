<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://saxon.sf.net/" extension-element-prefixes="saxon" version="2.0">
    ...
    <xsl:variable name="kd" select="document('../xml-from-sql/2017-04-06/tgn-results.xml')"/>
    ...
    <xsl:template match="row">
        ...
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="$kd/root/place[@entity=$join]">
                    <xsl:for-each select="$kd/root/place[@entity=$join][1]">
                        <xsl:copy-of select="*"/>
                    </xsl:for-each>
